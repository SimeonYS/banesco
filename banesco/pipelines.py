...
class BbanescoPipeline:

    # Database setup
    conn = sqlite3.connect('banesco.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `banesco`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM banesco WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO banesco (date, title, link, content)"
